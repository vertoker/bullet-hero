...
public class TimelineWindow : MonoBehaviour
{
    ...
    public void Load()
    {
        ...
        for (int i = 0; i < editor.prefabs.Count; i++)
        {
            ...
            tr.localPosition = new Vector2(Frame2Sec(editor.prefabs[i].startFrame) * 100, editor.prefabs[i].height * -50 - 50);
            tr.sizeDelta = new Vector2(100 * Frame2Sec(editor.prefabs[i].frameTimeExist), 50);
