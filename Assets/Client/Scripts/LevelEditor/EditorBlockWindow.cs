...
public class EditorBlockWindow : MonoBehaviour
{
    ...
    {
        secLength = Frame2Sec(prefab.frameTimeExist);
        secOffset = Frame2Sec(prefab.startFrame);
...
public class ObjectBlock
{
    ...
    public void Open(Prefab prefab)
    {
        ...
        startTime_IF.text = Frame2Sec(prefab.startFrame).ToString();
        lengthTime_IF.text = Frame2Sec(prefab.frameTimeExist).ToString();
        SelectSprite((int)prefab.st);
        //SelectAnchor((int)prefab.a);
    ...
    public Prefab Get()
    {
        int startTime = Sec2Frame(float.Parse(startTime_IF.text));
        int existTime = Sec2Frame(float.Parse(lengthTime_IF.text));
        return new Prefab(
            name_IF.text,
            active_T.isOn,
            collider_T.isOn,
            pos, sca, rot, clr,
            spriteType,
            startTime, existTime, startTime + existTime,
            Sec2Frame(float.Parse(startTime_IF.text)) + 
            0, 0, pivot, 0, 0, 0);
    }
    ...
    {
        return Mathf.FloorToInt(sec * 60f);
    }
}
...
public class ColorBlock
{
    ...
    public Clr GetColor()
    {
        return new Clr(
            float.Parse(time_IF.text),
            (ColorRandomType)idActive, EasingType.Linear,
