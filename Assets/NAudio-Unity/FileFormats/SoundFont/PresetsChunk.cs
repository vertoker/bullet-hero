...
namespace NAudio.SoundFont 
{
	...
	public class PresetsChunk 
	{
		...
		internal PresetsChunk(RiffChunk chunk) 
		{
			...
			if(header != "pdta") 
			{
				throw new System.IO.InvalidDataException(String.Format("Not a presets data chunk ({0})",header));
			}
			...
			while((c = chunk.GetNextSubChunk()) != null) 
			{
				...
				case "shdr":
					...
					break;
				default:
                    throw new System.IO.InvalidDataException(String.Format("Unknown chunk type {0}", c.ChunkID));
				}
			}
