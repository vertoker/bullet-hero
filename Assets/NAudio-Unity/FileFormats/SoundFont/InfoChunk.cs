...
namespace NAudio.SoundFont 
{
	...
	public class InfoChunk 
	{
	    internal InfoChunk(RiffChunk chunk) 
		{
			...
			{
				throw new System.IO.InvalidDataException("Not an INFO chunk");
			}
			//this.chunk = chunk;
			RiffChunk c;
			while((c = chunk.GetNextSubChunk()) != null) 
			{
				...
				case "ISFT":
					...
					break;
				default:
					throw new System.IO.InvalidDataException($"Unknown chunk type {c.ChunkID}");
				}
			}
			if(!ifilPresent) 
			{
                throw new System.IO.InvalidDataException("Missing SoundFont version information");
			}
            // n.b. issue #150 - it is valid for isng not to be present
			if(!inamPresent) 
			{
                throw new System.IO.InvalidDataException("Missing SoundFont name information");
			}
		}
